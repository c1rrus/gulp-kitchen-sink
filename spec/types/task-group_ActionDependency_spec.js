...
describe("An ActionDependency", function() {
  ...
  it("creates task names correctly", function(){
    expect(this.actionDep.createTaskName(this.tasks)).toEqual(GROUP_NAME + SEP + ACTION_NAME);
  });
  ...
  it("creates task names correctly (with prefix)", function(){
    ...
    expect(this.actionDep.createTaskName(this.tasks)).toEqual(prefixName + SEP + GROUP_NAME + SEP + ACTION_NAME);
  });

  it("creates task names correctly (with action before group)", function(){
    this.config.tasks.groupBeforeAction = false;
    expect(this.actionDep.createTaskName(this.tasks)).toEqual(ACTION_NAME + SEP + GROUP_NAME);
  });
  ...
  it("creates task names correctly (with prefix and action before group)", function(){
    ...
    this.config.tasks.defaultPrefixName = prefixName;
    this.config.tasks.groupBeforeAction = false;
    expect(this.actionDep.createTaskName(this.tasks)).toEqual(prefixName + SEP + ACTION_NAME + SEP + GROUP_NAME);
  });
