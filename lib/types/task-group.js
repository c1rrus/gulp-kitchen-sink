 ...
 * {@link module:gulp-kitchen-sink/types/build-config|build config object} to
 ...
 *                                        The build config whose settings should
 *                                        be used to generate the task name.
 *
 * @returns {string}  The complete task name.
 */
ActionDependency.prototype.taskName = function(config){
  return config.createTaskName(this.groupName, this.actionName);
};
 ...
 */
GulpTaskGroup.prototype.getActions = function(){
 ...
 *                              this group.
 */
GulpTaskGroup.prototype.actionDep = function(actionName){
 ...
 */
GulpTaskGroup.prototype.taskName = function(actionName){
 ...
 */
GulpTaskGroup.prototype.resolveDeps = function(deps){
  var i, dep;
  var resolvedDeps = [];
  for(i=0; i<deps.length; ++i){
    dep = deps[i];
    if(dep instanceof ActionDependency){
      resolvedDeps.push(dep.taskName(this.config));
    }
    else{
      // assume string action name from this group
      resolvedDeps.push(this.taskName(dep));
    }
  }
  return resolvedDeps;
};
...
GulpTaskGroup.prototype._createTaskLoaderFn = function(actionName, actionDeps, taskFn){
  ...
  return function(gulp){
    var taskName = this.taskName(actionName);
    if(actionDeps && actionDeps.length > 0){
      // Resolve dep names
      var taskDeps = this.resolveDeps(actionDeps);
      // Add task to Gulp
      gulp.task(
        taskName,
        taskDeps,
 ...
 *
 *
 * @param {string} fallbackGroupName      A fallback name that will be used for the task
 *                                        group, if none is provided when the returned
 ...
 */
GulpTaskGroup.createTasksModule = function(fallbackGroupName, addActionsFn){
  return function(gulp, groupName, config, lazyLoad){
    // Use the fallback group name, if none was provided
    if(!groupName){
