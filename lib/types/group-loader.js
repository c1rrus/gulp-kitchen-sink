 ...
 * @private
 */
GroupLoader.prototype._createModuleName = function(groupName){
 ...
 */
GroupLoader.prototype.taskGroup = function(groupName){
  var taskGroup = this._taskGroups[groupName];
  if(!taskGroup){
    taskGroup = this._taskGroups[groupName] = require(this._createModuleName(groupName))(this.gulp, groupName, this.config, true);
  }
  return taskGroup;
};
 ...
 * {@link module:gulp-kitchen-sink/types/group-loader#taskGroup|taskGroup()}
 * or
...
GroupLoader.prototype.availableTaskGroups = function(){
  var groupNames = [];
  var files = fs.readdirSync(this.modulesDir);
  var file, matches;
...
/**
 * Returns all available task groups, loading them from their modules if
 * required.
 *
 * The first time a task group is requested, its module will be loaded.
 * A reference to that task group will be cached internally, so that
...
GroupLoader.prototype.getAllAvailableTaskGroups = function(){
  ...
  for(var i=0; i<availableGroups.length; ++i){
    taskGroups.push(this.taskGroup(availableGroups[i]));
  }
  return taskGroups;
};
